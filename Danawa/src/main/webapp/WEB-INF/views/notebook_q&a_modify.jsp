...
<script src="./summernote-0.8.18-dist/lang/summernote-ko-KR.js"></script>
  <link rel="stylesheet" href="/resources/css/notebook_q&a_modify.css">
...
<body>
	...
	<main role="main" class="container">
		...
		<div id="subtitle" style="text-align: right;">
			<a href="./notebook_q&a_main.html"
				style="color: slateblue; text-decoration: underline;">노트북 Q&A</a> <i
				class="fas fa-angle-right"></i> <a href="./notebook_q&a_write.html"
				style="color: slateblue; text-decoration: underline;">글쓰기</a>
		</div>

		<div id="title_2"
			style="text-align: center; margin-bottom: 20px; margin-top: 30px; border-top: 1px solid lightgray;">
			<h4 id="title" style="margin-top: 30px;">수정하기</h4>
		</div>
      ...
      %>
		...
		<form class="form_1" name="form" method="post"
			action="/board/boardmodiffyok">
			<div class="pt-1"
				style="border: 1px solid gainsboro; width: 100%; height: 70px; display: flex; align-items: center;">
				<label style="padding-right: 20px; padding-left: 10px;">제목</label> <input
					type="text" name="subject" value="<%=bean.getSubject() %>"
					style="border-radius: 5px; width: 93%;">
			</div>

			<div class="pt-1"
				style="border: 1px solid gainsboro; width: 100%; height: 150px; display: flex; align-items: center;">
				<label style="padding-right: 20px; padding-left: 10px;">내용</label>
				<textarea rows="8" cols="145" name="content"><%=bean.getContent()%></textarea>
			</div>

			<input type="hidden" name="password" value="default">
			<!--게시물등록시 비밀번호설정을안했을시 default 라는값을 숨겨서 전달 -->
			...
			<input type="button" onclick="location.href ='notebook_q&a_main.jsp"
				value="전체게시글보기" id="">
		</form>
		<%} %>
